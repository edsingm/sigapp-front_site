    ...
    <section id="hero" className="relative -mt-16 overflow-hidden bg-[#0B1E39]">
      ...
      <div className="container-landing relative flex min-h-dvh flex-col justify-center py-24 sm:py-28 md:py-32">
        <div className="flex max-w-2xl flex-col gap-7 sm:gap-9 lg:max-w-132 xl:max-w-2xl">
          <span className="inline-flex w-fit max-w-full items-center gap-3 border-l-2 border-secondary pl-3 text-[11px] leading-tight font-bold tracking-[0.16em] text-secondary uppercase sm:text-xs">
          ...
          <h1 className="font-heading text-[2.35rem] leading-[0.98] font-semibold tracking-[-0.035em] text-white sm:text-5xl sm:leading-none md:text-6xl lg:text-6xl xl:text-7xl">
            Prospecte terrenos. <br className="hidden sm:block" />
            Calcule viabilidade. <br className="hidden sm:block" />
          ...
          <p className="max-w-[34ch] text-base leading-relaxed text-white/92 sm:max-w-[52ch] sm:text-lg">
            Da prospecção à legalização em um único sistema. DRE, TIR e fluxo de
            caixa em segundos — e aprovação no comitê em uma rodada, não quatro.
          </p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              variant="ghost-white"
              size="lg"
              className="h-12 w-full gap-2 rounded-md border border-white/16 bg-white/6 px-6 text-base sm:w-auto md:backdrop-blur-sm"
              nativeButton={false}
              render={
                <Link
