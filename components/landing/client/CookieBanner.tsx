...
import { Cookie, X, ChevronRight, Shield, BarChart2, Megaphone, Check } from "lucide-react"
...
const CATEGORIES: CategoryDef[] = [
  {
    ...
    label: "Funcionais",
    description:
      "Lembram suas preferências de interface — tema claro/escuro, último tenant acessado, estado do menu. Não requerem consentimento para os estritamente necessários, mas incluímos aqui para transparência.",
    required: false,
    icon: Shield,
  },
  {
...
function Toggle({
  ...
  return (
    <button
      ...
      className={cn(
        "relative inline-flex h-6 w-10 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2",
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground max-w-[55ch]">
                    Cookies essenciais garantem o funcionamento. Com seu consentimento, usamos
        ...
        <div className="fixed inset-x-4 bottom-4 z-70 mx-auto max-w-lg rounded-2xl border border-border bg-background shadow-2xl sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 sm:w-full">
          ...
          <div className="max-h-[60vh] overflow-y-auto px-5 py-4">
            <p className="mb-4 text-xs leading-relaxed text-muted-foreground">
              Escolha quais categorias de cookies você permite. Cookies estritamente necessários
              (autenticação, segurança, tema) não podem ser desativados pois são indispensáveis
            ...
            <div className="mb-3 flex items-start justify-between gap-4 rounded-xl border border-border bg-muted/30 p-3.5">
              <div className="flex items-start gap-2.5">
                <div className="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-md bg-primary/10">
                  <Shield className="size-3.5 text-primary" strokeWidth={1.75} />
                </div>
                <div>
                  <p className="text-xs font-semibold text-foreground">Estritamente necessários</p>
                  <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
                    Sessão, CSRF, tema. Indispensáveis — não é possível desativá-los.
                  </p>
                </div>
              </div>
